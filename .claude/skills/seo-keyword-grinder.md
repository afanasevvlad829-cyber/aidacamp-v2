...
- **Дешёвый черновик (опционально)** — `scripts/seo-cheap-draft.mjs` (Together AI, `Qwen/Qwen2.5-7B-Instruct-Turbo`, ~$0.3/1M токенов). Ключ `TOGETHER_API_KEY` — на сервере в `/opt/aistudio/.env`. См. правило применения в шаге 4.
...
8. **GEO-citability** — первый абзац хотя бы одной секции даёт прямой самодостаточный ответ на ключевой вопрос кластера (2–3 предложения, без «читайте ниже»): так его цитируют AI Overviews / ChatGPT / Perplexity. Подробнее — раздел «GEO».

**Опционально — дешёвый черновик для пунктов 1–6.** Для пунктов 1–6 (title/description/h1/subtitle/строка FAQ) — это вставка ключевого слова в уже существующий короткий текст, БЕЗ новых фактов и без творческой части. Такую механику можно предварительно черновить через `scripts/seo-cheap-draft.mjs`:
```bash
TOGETHER_API_KEY=<из /opt/aistudio/.env на сервере> node scripts/seo-cheap-draft.mjs \
  --keyword "<ключ кластера>" --text "<текущее значение поля>"
```
**Обязательно перед тем, как записать черновик в файл:**
- Прочитать `draft` глазами — модель дешёвая и иногда ошибается (грамматика, падежи).
- Проверить banned words и лимиты длины (title ≤60, description ≤140) — черновик их не знает.
- Проверить тон — брендовый голос (см. `agent_docs/brand-positioning.md`) черновик тоже не соблюдает.
- Если черновик не подходит — писать самому, не тратить вторую попытку на дешёвую модель ради экономии.

**НЕ использовать дешёвый черновик для:** пункта 7 (новые секции), GEO-абзаца (пункт 8), `create_page`, `expand_content` — там нужны новые факты/структура/творческая часть, а не вставка слова в готовый текст; это должен делать сам агент.
...
| Цена/путёвка | Таблица смен с ценами, налоговый вычет 13% (до 6 200 ₽, максимум — Смена 1), рассрочка |
...
## GEO — оптимизация под AI-поиск (AI Overviews / ChatGPT / Perplexity)

Классический ранкинг и цитируемость в ИИ-выдаче — не одно и то же. Основано на официальном Google AI-optimization guide (developers.google.com/search/docs/fundamentals/ai-optimization-guide): для ИИ-поиска SEO превращается в подготовку данных для RAG, а «AI-хаки» не работают.

**Что делаем на странице (в дополнение к обычному optimize):**
- **Прямой ответ в начале секции.** Первый абзац секции = самодостаточный ответ на вопрос (2–3 предложения, конкретика: км, цена, дни, возраст). AI цитирует абзацы, а не «прокрутите до таблицы».
- **Уникальная экспертиза, а не пересказ.** То, чего нет у конкурентов: конкретные API на сменах, реальные цифры трансфера/питания, наблюдения. Совпадает с нашей политикой уникальных текстов на каждом лендинге (`seasons.ts`).
- **Структурированность под query fan-out.** FAQ Schema покрывает не только основной ключ, но и смежные под-вопросы, которые ИИ дробит из одного запроса («сколько стоит», «с какого возраста», «как добраться», «есть ли вычет»).
- **Факты, которые легко извлечь.** Числа и условия — явным текстом рядом с вопросом, не только в картинке/таблице.

**Чего НЕ делаем:** `llms.txt`, скрытые промпты в контенте и прочие «AI-хаки» — Google прямо говорит, что не нужны; тратим усилие на экспертизу и структуру.

**Из сводки vc.ru/seo 08.2026 (`_notes/Библиотека знаний/AI-видимость и GEO — сводка vc.ru август 2026.md`):**
- FAQ как мини-карточка: вопрос языком пользователя → краткий ответ 1–2 предложения → развёрнутый → исключения отдельным блоком; блоки по сценариям («если…»), не по темам.
- AI-видимость мерить в трёх местах: Вебмастер «Видимость в Алисе AI», Bing «AI Performance» (Grounding Queries = чистые LSI), GSC «Генеративный ИИ». Движки не пересекаются (1,6–2,7% общих источников).
- Перед созданием новых страниц — сверка с лидерами ниши (keys.so): закрыты ли жирные хабы; главный запрос страницы проверять по частотности.
- **Очередь GEO-задач из выгрузки Алисы 02.09.2026** (детали и обоснование — в сводке): (1) таблица «норма экранного времени по возрастам 10–17» одним материалом; (2) формулировка «лагерь без телефонов» в H2/FAQ существующей страницы про телефоны — без новой страницы; (3) переупаковка FAQ «питание/правила/документы» в цитируемые карточки. Контроль — повторная выгрузка Алисы через 3–4 недели.

**Дополнительно (конспект Петрова, `_notes/Библиотека знаний/GEO и семантика — конспект Петрова.md`):**
- ИИ-агенты берут кандидатов из поисковых индексов (прежде всего Bing) — без топ-20 в ответы не попасть; GEO не заменяет обычное SEO.
- Первый экран / первые ~30% контента решают: прямой ответ и ключевые факты — вверху страницы, плотными списками/таблицами.
- ИИ-агенты не ждут медленный сервер (обрыв → 499 в nginx, в Метрике не видно) и не исполняют JS — статика Astro наш плюс, скорость ответа держим.
- Периодически проверять, что блок-лист скраперов не режет GPTBot/ClaudeBot/PerplexityBot/bingbot.
...
- Хардкодить устаревшие цифры вычета ("до 5 200 ₽", "до 5 434 ₽") — всегда "от 2 800 до 6 200 ₽ в зависимости от смены" (актуальные суммы — `_notes/Библиотека знаний/Налоговый вычет — правила и данные.md`)
...
- GEO: первый абзац ≥1 секции — прямой цитируемый ответ; FAQ покрывает смежные под-вопросы (см. раздел «GEO»)
- Черновик из `seo-cheap-draft.mjs` (если использовался) — ВСЕГДА проверен глазами перед записью в файл, никогда не применяется напрямую
