/* ===== JS Academy — Fun Pack v3 (revised) =====
...
(function(){
  ...
  const DONE_MSGS = [
    '🎉 Отличная работа!','🚀 Поехали дальше!','⭐ Супер!','💛 JavaScript гордится тобой!',
  ...
  const CAT_TIPS = {
    quiz: [
      ...
      'В JavaScript всегда ставь ; в конце строки — как точку в предложении 🐱',
    ...
    tasks: [
      ...
      'console.log() — твой лучший друг при отладке!',
    ...
    debug: [
      ...
      'ReferenceError? Проверь, не опечатался ли ты в имени переменной.',
      'SyntaxError? Проверь скобки {} и () — они должны идти парами.',
      'TypeError? Сравни типы: число и строка — не друзья без Number()/String().',
      'Когда застрял — встань, погладь кота, вернись к коду 🐱',
    ],
    lesson: [
      'Кодинг — это спорт для мозга. Тренируйся каждый день!',
      'Лучший способ выучить JavaScript — писать JavaScript каждый день 🚀',
  ...
  const MOTIVATIONS = [
    '🚀 Новая тема — новые суперсилы!','💛 Поехали покорять JavaScript!',
    '💡 Готов(а) узнать что-то крутое?','🎯 Сфокусируйся — и у тебя всё получится!',
    '⚡ Каждый шаг приближает тебя к мастерству!','🌟 Маленький джаваскриптер в работе!',
  ...
  function checkNewTopic(){
    ...
    if (!tabs && /Debug|AI|Достижения|Добро пожаловать|JS Academy/i.test(title)){
