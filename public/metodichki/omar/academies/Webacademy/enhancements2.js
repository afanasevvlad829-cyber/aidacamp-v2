/* ===== Python Academy — Fun Pack v2 (timers, sounds, walkers, profile) ===== */
(function(){
  'use strict';
  ...
  function openProfile(){
    ...
    const name       = (s.profile && s.profile.name) || 'Юный питонист';
    // levels: each 100 xp = 1 level
    ...
    back.innerHTML = `
      <div class="pa-modal" style="position:relative">
        ...
        <h2>👤 ${name}</h2>
        <div class="sub">Твоя личная карта прогресса в Python Academy</div>
        ...
        <div class="pa-section-title">До уровня ${level+1} — ещё ${nextXp - xp} XP</div>
    ...
    const close = ()=>{ window.PA_SFX && window.PA_SFX.swoosh && window.PA_SFX.swoosh(); back.remove(); };
