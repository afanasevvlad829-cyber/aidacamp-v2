/* ===== Python Academy — Themes (Light / Dark / Monokai) + Profile-in-main ===== */
(function(){
  'use strict';
...
(function(){
  function openHintPopup(html){
    ...
    overlay.innerHTML = `
      <div class="kt-hint-modal" role="dialog" aria-modal="true">
        <h3 class="kt-hint-title">💡 Подсказка</h3>
  ...
  document.addEventListener('click', function(e){
    ...
    // Find hint element: hidden <p class="muted"> or <ol>
    const hintEl = panel.querySelector('p.muted, ol');
    if (!hintEl) return;
    e.stopPropagation(); e.preventDefault();
    // Keep underlying element hidden
    hintEl.style.display = 'none';
    openHintPopup(hintEl.innerHTML || hintEl.textContent);
  }, true);
})();
