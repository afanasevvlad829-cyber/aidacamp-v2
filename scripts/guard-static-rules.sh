...
# цифра, а её подача как заголовочного максимума («до 5 200 ₽») литералом — максимум
# производится из VYCHET_MAX (src/data/shifts.ts) и меняется вместе с составом
# открытых смен, его нельзя фиксировать текстом ни на какое число. Отличить
# посменную сумму от заголовочной может только смысл: сейчас 5 200 стоит в
# src/scripts/pages/ask.ts и src/lib/ai/campData.ts именно как посменная — то есть
# корректно. Добавишь «5 200» сюда — страж начнёт врать на верном коде, его
# выключат, а вместе с ним и доверие к остальным. Это семантика, не греп.
            ...
            for i, line in enumerate(fp, 1):
                if STALE_DEDUCTION.search(line) and re.search(r'вычет|₽|руб', line, re.IGNORECASE):
                    errors.append(
                        ...
                        f'     {line.strip()[:110]}\n'
                        f'     CLAUDE.md называет 5 434 / 9 737 устаревшими. Максимум — VYCHET_MAX\n'
                        f'     из shifts.ts (не литерал — меняется с составом открытых смен),\n'
