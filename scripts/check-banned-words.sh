...
#
# Использует node (fallback с python3 если node недоступен).
...
const SOFT = /\bдет(?:и|ей|ям|ях|ьми)\b|\bребён(?:ок|ка|ку|ком|ке)\b|\bребенок\b/gi;
...
for (const base of SCAN) {
  walkDir(base, (filePath) => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (m) fileHits.push({ ln: i + 1, word: m[0], text: line.trim().slice(0, 120) });
    }
