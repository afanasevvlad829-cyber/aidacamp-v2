...
#
# ⏰ ВРЕМЯ ЗАПУСКА ВАЖНО: 10:00 UTC (13:00 МСК), то есть ПОСЛЕ конвейера seo-wave-cycle
# (07:00 UTC, работает до ~2,5 ч). Инцидент 21.08.2026: отчёт стоял на 06:00 UTC — на час
# РАНЬШЕ конвейера, поэтому каждый день показывал состояние «до работы»: владелец видел
# «затронуто буквально пару страниц», хотя в тот день конвейер сделал 19 волн. Если будешь
# менять расписание — держи отчёт позже цикла, иначе он снова начнёт врать.
...
<h1>🧭 SEO-пульс</h1>
<p class="muted">Ежедневный срез: правки за сутки, конкуренты/частотность, волна цикла (фронт A). Прогон: ${NOW_H}. Cron: ежедневно 13:00 МСК — ПОСЛЕ прогона конвейера (10:00 МСК, идёт до ~2,5 ч), чтобы отчёт показывал результат дня, а не состояние до него. Лог: /var/log/seo-pulse.log</p>
