...
3. **конверсионный smoke прода** (`scripts/smoke-conversion.sh`, вызывается из
   `deploy.sh prod` после обычного smoke) — Метрика реально инициализируется
   ...
   сломан, откатывай» + авто-откат. Ловит класс инцидента Partytown
...
4. **авто-откат** на последний `backup-*`, если верификация или smoke прода провалились.
