...
Документы для заезда (Приложение №1 к договору, везти оригиналы + копии): 1) копия свидетельства о рождении / паспорта ребёнка; 2) копия полиса ОМС; 3) медсправка 079/у (срок 3 мес.); 4) справка об отсутствии контактов с инфекционными больными (срок 3 дня, датировать за 3 дня до заезда); 5) согласие на обработку персональных данных (Приложение №2, подписывается с договором); 6) справка для бассейна (можно не брать отдельно, если отметка о бассейне есть в 079/у). Также Дарья просит заполненную анкету участника и копию паспорта одного из родителей. Передаются ОРИГИНАЛЫ лично вожатому на посадке/при приезде — заранее сканы НЕ присылают, после смены пакет возвращают. Не путать с данными для ЗАКЛЮЧЕНИЯ ДОГОВОРА (нужны заранее текстом: ФИО/паспорт заказчика, ФИО/ДР/свидетельство ребёнка).
