...
| Файл | Тип страницы | Форма | Особенности |
|---|---|---|---|
| `03-filial.html` | Страница офлайн-филиала | возраст + телефон | счётчик мест, карта, преподаватели |
| `04-seo-vozrast.html` | SEO по возрасту (10–12 лет) | курс + формат + телефон | переключатель возрастов (3 таба) |
| `05-seo-metro.html` | SEO по метро (онлайн-фокус) | курс + телефон | инфо о ближайшем офлайне |
| `06-seo-tema.html` | Каталог всех курсов | возраст + формат + телефон | фильтры по возрасту, 18 курсов |
| `07-article.html` | Статья / блог | телефон в сайдбаре | sticky sidebar, blockquote, таблица |
| `08-stati-list.html` | Список статей | телефон в сайдбаре | featured + сетка 2×2, фильтры |
| `09-promo-landing.html` | Промо-лендинг (МК/акция) | имя + возраст + формат + телефон | без navbar/footer, счётчик мест |
| `10-teacher.html` | Карточка преподавателя | курс + телефон | аватар, достижения, отзывы |
| `11-technicka.html` | О школе / документы | нет | sticky tabs, FAQ-коллапсы |

### Варианты формы по шаблонам

| Поле | filial | seo-age | seo-metro | seo-tema | promo |
|---|---|---|---|---|---|
| Имя ребёнка | — | — | — | — | ✅ |
| Возраст (3 кнопки) | ✅ | — | — | ✅ | ✅ |
| Курс (dropdown) | — | ✅ | ✅ | — | — |
...
### Уникальные UI-элементы по шаблонам

**Филиал (`03`):**
- Бейджи мест: зелёный «Есть места» / красный «2 места»
- Sticky форма в сайдбаре

**SEO по возрасту (`04`):**
- Переключатель `/7-9let` | `/10-12let` | `/13-15let` в header
- Карточки курсов на тёмном фоне

**Промо-лендинг (`09`):**
- Пульсирующий индикатор (CSS animation)
- Счётчик мест — 8 квадратиков, часть заполнена
- Step-номера в оранжевых квадратах (моноширинный шрифт)
- Полноэкранный hero без стандартного Header/Footer

**Статья (`07`):**
- `.article-body` — расширенная типография
- Blockquote с оранжевой полосой слева
- Highlight-блок с оранжевым left-border
- Sticky sidebar `top: 80px`

**О школе (`11`):**
- Sticky табы (О школе / Контакты / Цены / Документы / FAQ)
- FAQ-коллапсы: иконка `+` вращается 45° при открытии

### Цветовые дополнения (сверх основной палитры)

| Контекст | Цвет | Где |
|---|---|---|
| «Есть места» | зелёный `#dcfce7` | бейдж филиала |
| Иконки статей | синий, фиолетовый, зелёный | категории блога |
| Документы | оранжевый, синий, зелёный, серый | карточки документов |
| Промо shapes | синий декоративный | прото-лендинг фон |
