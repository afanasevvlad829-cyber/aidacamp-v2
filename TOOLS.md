...
## 🧠 БАЗА ЗНАНИЙ — когда и как использовать

**RAG-хранилище:** PostgreSQL таблица `knowledge_chunks` на сервере.  
**Доступ через MCP:** `run(service="kb", action="search", params={query, mode, limit})`  
**Короткий alias** (в глобальном CLAUDE.md): `kb_search(query, mode="hybrid")`

### Когда вызывать ПЕРВЫМ делом

| Тип задачи | Действие |
|---|---|
| Написать текст / объявление / пост | `kb_search` за позиционированием и tone of voice |
| SEO-задача (стратегия, контент, аудит) | `kb_search` за экспертизой и правилами |
| Реклама Директ / VK | `kb_search` за историей стратегий и ограничениями |
| "Что мы решили по X?" / "Как мы делаем Y?" | `kb_search` |
| Живые данные (позиции, статистика, лиды) | **напрямую в API** — kb_search не нужен |
| Код / файлы сайта | **Read/Grep** — kb_search не нужен |

### Что хранится

| source-префикс | Содержимое |
|---|---|
| `notes:*` | `_notes/**/*.md` — вся экспертиза, стратегии, регламенты (2475 чанков) |
| `claude_session:*` | Диалоги из сессий Claude Code — решения, инсайты |
| `https://aidacamp.ru*` | Страницы сайта (контент, структура) |

### Примеры запросов

```python
# Тон голоса и позиционирование
kb_search("tone of voice Дарья подруга мама")

# SEO-правила для Яндекса
kb_search("факторы ранжирования Яндекс 2026 поведенческие")

# История решений по рекламе
kb_search("стратегия VK кампания результат")

# Портрет клиента
kb_search("портрет клиента мама страхи мотивация")
```

### Режимы поиска
- `hybrid` — по смыслу + по ключевым словам (рекомендуется)
- `semantic` — только векторный поиск
- `fulltext` — только текстовый

### Автообновление
`~/MCP/notes_indexer.py` — запускать при изменении `_notes/`, или через LaunchAgent ежедневно.
...
`/opt/scripts/replicate-poll.sh <outdir> <id...>` (написан и проверен 13.07.2026) сам ждёт готовности (интервал 15с, потолок 30 мин), сам скачивает готовое видео в `<outdir>/<id>.mp4`, пишет статус в лог. Токен ищет тем же способом, что уже принят в проекте (`r8_...` в `/opt/*/.env*`).
...
7. **Запрет на галлюцинации — НЕ СРЕЗАЙ УГЛЫ**

   Если задача требует актуальных данных из внешнего источника (сайт, API, файл, сервер) — **иди за данными**. Не используй:
   - информацию из контекста разговора как замену реальным данным
   - предположения вместо фактических значений
   - «помню что было» вместо свежих данных

   **Правило трёх шагов:**
   - Есть инструмент для получения данных → **вызови инструмент**
   - Нет инструмента, но есть SSH/browser_agent → **иди через них**
   - Ни того ни другого → **задай уточняющий вопрос пользователю**

   ❌ НЕЛЬЗЯ: «Сайт aidacamp.ru показывает X» (без реального scrape)
   ❌ НЕЛЬЗЯ: «Судя по нашему разговору, там написано Y»
   ✅ НУЖНО: вызвать `browser_agent(action="scrape", url="...")` и цитировать результат
   ✅ НУЖНО: если данных нет — сказать «Мне нужно [что именно] — уточни или я схожу проверю»
