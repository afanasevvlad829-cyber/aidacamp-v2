...
Рациональный аргумент для папы. Документы для вычета даём автоматически. **Конкретные суммы растут вместе с ценой смены** — актуальная полная таблица только в CLAUDE.md → «Налоговый вычет» (не дублировать литералы здесь, сверять при изменении цен). Ориентир на сезон 2026:
