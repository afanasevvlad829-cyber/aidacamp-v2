...
> ⚠️ **Единый источник цен — `src/data/shifts.ts`.** Build-страж `npm run check:prices` роняет сборку при любой цене смены вне источника. Суммы вычета ниже — ориентир (зависят от образовательной части путёвки), сверять отдельно при изменении цен.
