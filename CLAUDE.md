...
## 🚨 КРИТИЧЕСКОЕ ПРАВИЛО — GIT ВЕТКИ (нарушение = потеря работы)

> **Это правило нарушалось несколько раз подряд. Читай до начала любой задачи.**

### ЗАПРЕЩЕНО: коммитить напрямую в `dev`

Даже одна правка одного файла — сначала создай ветку.

### Правило для КАЖДОЙ задачи (без исключений):

```bash
git fetch origin && git pull --rebase origin dev
git checkout -b agent/<задача>          # ← ОБЯЗАТЕЛЬНО
# ... делаешь работу ...
git add <файлы> && git commit -m "..."
git push -u origin agent/<задача>
gh pr create --base dev --title "..."   # ← PR, не мёрдж
```

Дальше **ждёшь** — владелец сам мёрджит PR в `dev`.

### ❌ Никогда:
- `git commit` находясь на `dev`
- `git push origin dev`
- `git merge` или `git rebase` в `dev` напрямую

### ✅ Имена веток:
`agent/hero-fixes`, `agent/seo-meta`, `agent/new-page-lager`, `agent/schema-review`
