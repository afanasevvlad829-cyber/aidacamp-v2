...
**Любая задача по коду / сайту / скриптам запускается ТОЛЬКО через:**

```bash
./scripts/vps-start-agent.sh "<задача>"
```

Скрипт поднимает агента на VPS в `~/Aidacamp`, в **изолированном git-worktree** (через `agent-start.sh`), в tmux-сессии: переживает отключение, стримит лог обратно, в конце — `AGENT_DONE` + ссылка на PR.

**🚫 Запрещено:**
- Редактировать `/opt/*` на сервере напрямую — теряется, не в git (инцидент 06-06: скрипты фабрики терялись).
- Вести разработку в локальном Mac-чекауте — грязное дерево блокирует деплой, `/tmp` теряется при компактах.
- Ad-hoc `ssh … && edit` вместо изолированного агента — угоняет ветку в общем чекауте (инцидент 06-06: ночной крон переключил ветку под ручной работой).

**Почему:** изоляция worktree (ветку никто не угоняет), работа на сервере (контекст не теряется при компактах), всё через git → PR в dev. Деплой прода — отдельно, из чистого worktree владельцем.
