...
|---|---|---|
| `единиц` / `единицы` / `единицами` | дешёвый игровой жаргон, ломает премиум-позицию | `монет` / `монеты` (название игровой валюты лагеря) |
| `баллов` / `баллы` / `балла` / `балл` | школьный оценочный жаргон, обесценивает игру | `монет` / `монеты` |
| `игровая валюта` / `игровые рубли` / `игровых рублей` | прямолинейный геймификацион-сленг | `монеты` (с пояснением «как 200 рублей в Монополии») |
...
**Контроль:** скрипт `scripts/check-banned-words.sh` (вызывается из `npm run build` через guard). Найдено `единиц`/`баллов`/«игровая валюта» — билд падает.
