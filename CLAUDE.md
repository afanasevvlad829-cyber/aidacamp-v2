...
- **Иконки:** используем **Bootstrap Icons** (CDN подключён в head, `<i class="bi bi-..."></i>`).
  Полный каталог: https://icons.getbootstrap.com/
- **Эмодзи:** НЕ используем в интерфейсе (ни в текстах кнопок, ни в заголовках, ни в карточках). Только Bootstrap Icons или inline-SVG.
  Исключение — текст самого контента статьи, где эмодзи является частью цитаты/примера.
