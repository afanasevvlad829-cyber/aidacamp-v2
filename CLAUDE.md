...
❌ ЗАПРЕЩЕНО: писать "до 5 434 ₽" — устаревшая цифра. Также запрещено хардкодить
любую сумму (включая «до 6 200 ₽» или «до 5 200 ₽») как «максимум» текстом —
максимум меняется вместе с составом открытых смен (`mainShifts`).
✅ Если нужна одна цифра — брать `VYCHET_MAX` из `src/data/shifts.ts` (производный
максимум по открытым сменам), не литерал. Сейчас это 5 200 ₽ (Смены 3/4 открыты,
Смена 1 закрыта) — при следующей смене состава сезона число изменится само.
