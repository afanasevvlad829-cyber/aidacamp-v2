...
> **⚡ Правило №1 (разработка) — по риску, не по умолчанию.** Решение принято 03.08.2026: полная Docker-изоляция — не для каждой правки, а только там, где цена ошибки реально высокая.
>
> - **Docker-контейнер обязателен**, если правка трогает: деньги/платежи (T-Bank и т.п.), персональные данные (дети, родители, контакты), прод-доступ/секреты, или что-то, что не жалко сломать по-крупному. Запуск: `./scripts/agent-docker.sh "<задача>" "<бриф>"` — агент клонирует репо из GitHub в одноразовый non-root контейнер, прод/секреты ему недоступны. Подробности — [`DEV_PROTOCOL.md`](DEV_PROTOCOL.md) → «Правило №1» + [`docker/agent/README.md`](docker/agent/README.md).
> - **Мелкие правки** (UI, вёрстка, безобидный SQL-фикс, косметика) — можно быстрее и без контейнера: headless `claude -p` на сервере от **non-root пользователя `claude-run`** (`ssh aidacamp`, дальше `sudo -u claude-run ...`), уже настроен и авторизован. Не под root, репо клонировать в его домашнюю папку, а не трогать `/opt/aidacamp-site` напрямую (там root-owned).
> - Локально на маке — только хотфиксы владельца.
> - ⚠️ Старый `vps-start-agent.sh` (агент под root прямо на проде, без изоляции вообще) — **DEPRECATED**, небезопасен, не путать с `claude-run`.
