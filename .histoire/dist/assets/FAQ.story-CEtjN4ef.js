import { au as openBlock, av as createBlock, aw as defineComponent } from "./vendor-C3NrspRX.js";
import { _ as _export_sfc, B as BlockFrame } from "./BlockFrame-BowLW7Bk.js";
const _sfc_main = /* @__PURE__ */ defineComponent({
  __name: "FAQ.story",
  setup(__props, { expose: __expose }) {
    __expose();
    const __returned__ = { BlockFrame };
    Object.defineProperty(__returned__, "__isScriptSetup", { enumerable: false, value: true });
    return __returned__;
  }
});
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return openBlock(), createBlock($setup["BlockFrame"], {
    id: "blocks-faq--default",
    title: "Blocks / FAQ",
    height: 960
  });
}
_sfc_main.__file = "src/histoire/FAQ.story.vue";
const FAQ_story = /* @__PURE__ */ _export_sfc(_sfc_main, [["render", _sfc_render], ["__file", "/root/aidaplus-dev-serverwork/src/histoire/FAQ.story.vue"]]);
export {
  FAQ_story as default
};
